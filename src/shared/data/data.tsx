export const places = [
  { english: "Airport", native: "สนามบิน", pronunciation: "sà naam bin" },
  { english: "Museum", native: "พิพิธภัณฑ์", pronunciation: "pí pít thá pan" },
  { english: "Bank", native: "ธนาคาร", pronunciation: "thá naa kaan" },
  {
    english: "Gas station",
    native: "ปั๊มน้ำมัน",
    pronunciation: "pàm náam man",
  },
  {
    english: "Bookstore",
    native: "ร้านหนังสือ",
    pronunciation: "ráan nǎng sʉ̌ʉ",
  },
  {
    english: "Police station",
    native: "สถานีตำรวจ",
    pronunciation: "sà thǎa nii tam rùat",
  },
  {
    english: "Clothing store",
    native: "ร้านเสื้อผ้า",
    pronunciation: "ráan sûea phâa",
  },
  {
    english: "Railway station",
    native: "สถานีรถไฟ",
    pronunciation: "sà thǎa nii rót fai",
  },
  { english: "Restaurant", native: "ร้านอาหาร", pronunciation: "ráan aa hǎan" },
  { english: "Cinema", native: "โรงหนัง", pronunciation: "roong nǎng" },
  { english: "Temple", native: "วัด", pronunciation: "wát" },
  {
    english: "Hospital",
    native: "โรงพยาบาล",
    pronunciation: "roong phá yaa baan",
  },
  { english: "House / Home", native: "บ้าน", pronunciation: "bâan" },
  { english: "School", native: "โรงเรียน", pronunciation: "roong riian" },
  { english: "Beach", native: "ทะเล", pronunciation: "thá le" },
  { english: "Hotel", native: "โรงแรม", pronunciation: "roong ram" },
  { english: "Market", native: "ตลาดใน", pronunciation: "tà làat nai" },
  { english: "Library", native: "ห้องสมุด", pronunciation: "hông sà mùt" },
];

export const numbers20to100 = [
  { num: 20, thaiNum: "๒๐", thaiText: "ยี่สิบ", eng: "yîi sìp" },
  { num: 21, thaiNum: "๒๑", thaiText: "ยี่สิบเอ็ด", eng: "yîi sìp èt" },
  { num: 22, thaiNum: "๒๒", thaiText: "ยี่สิบสอง", eng: "yîi sìp sǒng" },
  { num: 23, thaiNum: "๒๓", thaiText: "ยี่สิบสาม", eng: "yîi sìp sǎam" },
  { num: 24, thaiNum: "๒๔", thaiText: "ยี่สิบสี่", eng: "yîi sìp sìi" },
  { num: 25, thaiNum: "๒๕", thaiText: "ยี่สิบห้า", eng: "yîi sìp hâa" },
  { num: 26, thaiNum: "๒๖", thaiText: "ยี่สิบหก", eng: "yîi sìp hòk" },
  { num: 27, thaiNum: "๒๗", thaiText: "ยี่สิบเจ็ด", eng: "yîi sìp jèt" },
  { num: 28, thaiNum: "๒๘", thaiText: "ยี่สิบแปด", eng: "yîi sìp bpàet" },
  { num: 29, thaiNum: "๒๙", thaiText: "ยี่สิบเก้า", eng: "yîi sìp gâo" },
  { num: 30, thaiNum: "๓๐", thaiText: "สามสิบ", eng: "sǎam sìp" },
  { num: 31, thaiNum: "๓๑", thaiText: "สามสิบเอ็ด", eng: "sǎam sìp èt" },
  { num: 32, thaiNum: "๓๒", thaiText: "สามสิบสอง", eng: "sǎam sìp sǒng" },
  { num: 33, thaiNum: "๓๓", thaiText: "สามสิบสาม", eng: "sǎam sìp sǎam" },
  { num: 34, thaiNum: "๓๔", thaiText: "สามสิบสี่", eng: "sǎam sìp sìi" },
  { num: 35, thaiNum: "๓๕", thaiText: "สามสิบห้า", eng: "sǎam sìp hâa" },
  { num: 36, thaiNum: "๓๖", thaiText: "สามสิบหก", eng: "sǎam sìp hòk" },
  { num: 37, thaiNum: "๓๗", thaiText: "สามสิบเจ็ด", eng: "sǎam sìp jèt" },
  { num: 38, thaiNum: "๓๘", thaiText: "สามสิบแปด", eng: "sǎam sìp bpàet" },
  { num: 39, thaiNum: "๓๙", thaiText: "สามสิบเก้า", eng: "sǎam sìp gâo" },
  { num: 40, thaiNum: "๔๐", thaiText: "สี่สิบ", eng: "sìi sìp" },
  { num: 41, thaiNum: "๔๑", thaiText: "สี่สิบเอ็ด", eng: "sìi sìp èt" },
  { num: 42, thaiNum: "๔๒", thaiText: "สี่สิบสอง", eng: "sìi sìp sǒng" },
  { num: 43, thaiNum: "๔๓", thaiText: "สี่สิบสาม", eng: "sìi sìp sǎam" },
  { num: 44, thaiNum: "๔๔", thaiText: "สี่สิบสี่", eng: "sìi sìp sìi" },
  { num: 45, thaiNum: "๔๕", thaiText: "สี่สิบห้า", eng: "sìi sìp hâa" },
  { num: 46, thaiNum: "๔๖", thaiText: "สี่สิบหก", eng: "sìi sìp hòk" },
  { num: 47, thaiNum: "๔๗", thaiText: "สี่สิบเจ็ด", eng: "sìi sìp jèt" },
  { num: 48, thaiNum: "๔๘", thaiText: "สี่สิบแปด", eng: "sìi sìp bpàet" },
  { num: 49, thaiNum: "๔๙", thaiText: "สี่สิบเก้า", eng: "sìi sìp gâo" },
  { num: 50, thaiNum: "๕๐", thaiText: "ห้าสิบ", eng: "hâa sìp" },
  { num: 51, thaiNum: "๕๑", thaiText: "ห้าสิบเอ็ด", eng: "hâa sìp èt" },
  { num: 52, thaiNum: "๕๒", thaiText: "ห้าสิบสอง", eng: "hâa sìp sǒng" },
  { num: 53, thaiNum: "๕๓", thaiText: "ห้าสิบสาม", eng: "hâa sìp sǎam" },
  { num: 54, thaiNum: "๕๔", thaiText: "ห้าสิบสี่", eng: "hâa sìp sìi" },
  { num: 55, thaiNum: "๕๕", thaiText: "ห้าสิบห้า", eng: "hâa sìp hâa" },
  { num: 56, thaiNum: "๕๖", thaiText: "ห้าสิบหก", eng: "hâa sìp hòk" },
  { num: 57, thaiNum: "๕๗", thaiText: "ห้าสิบเจ็ด", eng: "hâa sìp jèt" },
  { num: 58, thaiNum: "๕๘", thaiText: "ห้าสิบแปด", eng: "hâa sìp bpàet" },
  { num: 59, thaiNum: "๕๙", thaiText: "ห้าสิบเก้า", eng: "hâa sìp gâo" },
  { num: 60, thaiNum: "๖๐", thaiText: "หกสิบ", eng: "hòk sìp" },
  { num: 61, thaiNum: "๖๑", thaiText: "หกสิบเอ็ด", eng: "hòk sìp èt" },
  { num: 62, thaiNum: "๖๒", thaiText: "หกสิบสอง", eng: "hòk sìp sǒng" },
  { num: 63, thaiNum: "๖๓", thaiText: "หกสิบสาม", eng: "hòk sìp sǎam" },
  { num: 64, thaiNum: "๖๔", thaiText: "หกสิบสี่", eng: "hòk sìp sìi" },
  { num: 65, thaiNum: "๖๕", thaiText: "หกสิบห้า", eng: "hòk sìp hâa" },
  { num: 66, thaiNum: "๖๖", thaiText: "หกสิบหก", eng: "hòk sìp hòk" },
  { num: 67, thaiNum: "๖๗", thaiText: "หกสิบเจ็ด", eng: "hòk sìp jèt" },
  { num: 68, thaiNum: "๖๘", thaiText: "หกสิบแปด", eng: "hòk sìp bpàet" },
  { num: 69, thaiNum: "๖๙", thaiText: "หกสิบเก้า", eng: "hòk sìp gâo" },
  { num: 70, thaiNum: "๗๐", thaiText: "เจ็ดสิบ", eng: "jèt sìp" },
  { num: 71, thaiNum: "๗๑", thaiText: "เจ็ดสิบเอ็ด", eng: "jèt sìp èt" },
  { num: 72, thaiNum: "๗๒", thaiText: "เจ็ดสิบสอง", eng: "jèt sìp sǒng" },
  { num: 73, thaiNum: "๗๓", thaiText: "เจ็ดสิบสาม", eng: "jèt sìp sǎam" },
  { num: 74, thaiNum: "๗๔", thaiText: "เจ็ดสิบสี่", eng: "jèt sìp sìi" },
  { num: 75, thaiNum: "๗๕", thaiText: "เจ็ดสิบห้า", eng: "jèt sìp hâa" },
  { num: 76, thaiNum: "๗๖", thaiText: "เจ็ดสิบหก", eng: "jèt sìp hòk" },
  { num: 77, thaiNum: "๗๗", thaiText: "เจ็ดสิบเจ็ด", eng: "jèt sìp jèt" },
  { num: 78, thaiNum: "๗๘", thaiText: "เจ็ดสิบแปด", eng: "jèt sìp bpàet" },
  { num: 79, thaiNum: "๗๙", thaiText: "เจ็ดสิบเก้า", eng: "jèt sìp gâo" },
  { num: 80, thaiNum: "๘๐", thaiText: "แปดสิบ", eng: "bpàet sìp" },
  { num: 81, thaiNum: "๘๑", thaiText: "แปดสิบเอ็ด", eng: "bpàet sìp èt" },
  { num: 82, thaiNum: "๘๒", thaiText: "แปดสิบสอง", eng: "bpàet sìp sǒng" },
  { num: 83, thaiNum: "๘๓", thaiText: "แปดสิบสาม", eng: "bpàet sìp sǎam" },
  { num: 84, thaiNum: "๘๔", thaiText: "แปดสิบสี่", eng: "bpàet sìp sìi" },
  { num: 85, thaiNum: "๘๕", thaiText: "แปดสิบห้า", eng: "bpàet sìp hâa" },
  { num: 86, thaiNum: "๘๖", thaiText: "แปดสิบหก", eng: "bpàet sìp hòk" },
  { num: 87, thaiNum: "๘๗", thaiText: "แปดสิบเจ็ด", eng: "bpàet sìp jèt" },
  { num: 88, thaiNum: "๘๘", thaiText: "แปดสิบแปด", eng: "bpàet sìp bpàet" },
  { num: 89, thaiNum: "๘๙", thaiText: "แปดสิบเก้า", eng: "bpàet sìp gâo" },
  { num: 90, thaiNum: "๙๐", thaiText: "เก้าสิบ", eng: "gâo sìp" },
  { num: 91, thaiNum: "๙๑", thaiText: "เก้าสิบเอ็ด", eng: "gâo sìp èt" },
  { num: 92, thaiNum: "๙๒", thaiText: "เก้าสิบสอง", eng: "gâo sìp sǒng" },
  { num: 93, thaiNum: "๙๓", thaiText: "เก้าสิบสาม", eng: "gâo sìp sǎam" },
  { num: 94, thaiNum: "๙๔", thaiText: "เก้าสิบสี่", eng: "gâo sìp sìi" },
  { num: 95, thaiNum: "๙๕", thaiText: "เก้าสิบห้า", eng: "gâo sìp hâa" },
  { num: 96, thaiNum: "๙๖", thaiText: "เก้าสิบหก", eng: "gâo sìp hòk" },
  { num: 97, thaiNum: "๙๗", thaiText: "เก้าสิบเจ็ด", eng: "gâo sìp jèt" },
  { num: 98, thaiNum: "๙๘", thaiText: "เก้าสิบแปด", eng: "gâo sìp bpàet" },
  { num: 99, thaiNum: "๙๙", thaiText: "เก้าสิบเก้า", eng: "gâo sìp gâo" },
  { num: 100, thaiNum: "๑๐๐", thaiText: "หนึ่งร้อย", eng: "nùeng rói" },
];

export const numbers0to20 = [
  { num: 0, thaiNum: "๐", thaiText: "ศูนย์", eng: "soon" },
  { num: 1, thaiNum: "๑", thaiText: "หนึ่ง", eng: "nùeng" },
  { num: 2, thaiNum: "๒", thaiText: "สอง", eng: "sǒng" },
  { num: 3, thaiNum: "๓", thaiText: "สาม", eng: "sǎam" },
  { num: 4, thaiNum: "๔", thaiText: "สี่", eng: "sìi" },
  { num: 5, thaiNum: "๕", thaiText: "ห้า", eng: "hâa" },
  { num: 6, thaiNum: "๖", thaiText: "หก", eng: "hòk" },
  { num: 7, thaiNum: "๗", thaiText: "เจ็ด", eng: "jèt" },
  { num: 8, thaiNum: "๘", thaiText: "แปด", eng: "bpàet" },
  { num: 9, thaiNum: "๙", thaiText: "เก้า", eng: "gâo" },
  { num: 10, thaiNum: "๑๐", thaiText: "สิบ", eng: "sìp" },
  { num: 11, thaiNum: "๑๑", thaiText: "สิบเอ็ด", eng: "sìp èt" },
  { num: 12, thaiNum: "๑๒", thaiText: "สิบสอง", eng: "sìp sǒng" },
  { num: 13, thaiNum: "๑๓", thaiText: "สิบสาม", eng: "sìp sǎam" },
  { num: 14, thaiNum: "๑๔", thaiText: "สิบสี่", eng: "sìp sìi" },
  { num: 15, thaiNum: "๑๕", thaiText: "สิบห้า", eng: "sìp hâa" },
  { num: 16, thaiNum: "๑๖", thaiText: "สิบหก", eng: "sìp hòk" },
  { num: 17, thaiNum: "๑๗", thaiText: "สิบเจ็ด", eng: "sìp jèt" },
  { num: 18, thaiNum: "๑๘", thaiText: "สิบแปด", eng: "sìp bpàet" },
  { num: 19, thaiNum: "๑๙", thaiText: "สิบเก้า", eng: "sìp gâo" },
  { num: 20, thaiNum: "๒๐", thaiText: "ยี่สิบ", eng: "yîi sìp" },
];

export const pronouns = [
  { english: "I / me (female speaker)", native: "ฉัน", pronunciation: "chǎn" },
  { english: "I / me (male speaker)", native: "ผม", pronunciation: "phǒm" },
  { english: "You", native: "คุณ", pronunciation: "khun" },
  { english: "He / she / they", native: "เขา", pronunciation: "kháo" },
  { english: "It (animal/object)", native: "มัน", pronunciation: "man" },
  { english: "They / them", native: "พวกเขา", pronunciation: "phûak-kháo" },
  { english: "We (inclusive)", native: "พวกเรา", pronunciation: "phûak-rao" },
  { english: "She (casual)", native: "เธอ", pronunciation: "thoe" },
  { english: "Himself / herself", native: "เขาเอง", pronunciation: "kháo-eng" },
  { english: "Oneself", native: "ตัวเอง", pronunciation: "tua-eng" },
];

export const adjectives1 = [
  { english: "Good", native: "ดี", pronunciation: "dee" },
  { english: "Bad", native: "แย่", pronunciation: "yâe" },
  { english: "Big", native: "ใหญ่", pronunciation: "yài" },
  { english: "Small", native: "เล็ก", pronunciation: "lék" },
  { english: "Difficult", native: "ยาก", pronunciation: "yâak" },
  { english: "Easy", native: "ง่าย", pronunciation: "ngâai" },
  { english: "Delicious", native: "อร่อย", pronunciation: "a-ròi" },
  { english: "Short (things)", native: "สั้น", pronunciation: "sân" },
  { english: "Short (people)", native: "เตี้ย", pronunciation: "tîa" },
  { english: "Tall", native: "สูง", pronunciation: "sǔung" },
  { english: "New", native: "ใหม่", pronunciation: "mài" },
  { english: "Old (things)", native: "เก่า", pronunciation: "kào" },
  { english: "Old (people)", native: "แก่", pronunciation: "kàe" },
  { english: "Hungry", native: "หิว", pronunciation: "hĭw" },
  { english: "Hot", native: "ร้อน", pronunciation: "rórn" },
  { english: "Cold (weather)", native: "หนาว", pronunciation: "năao" },
  { english: "Cold (everything)", native: "เย็น", pronunciation: "yen" },
  { english: "Cheap", native: "ถูก", pronunciation: "tùuk" },
  { english: "Expensive", native: "แพง", pronunciation: "paeng" },
  { english: "Clean", native: "สะอาด", pronunciation: "sà-àat" },
  { english: "Dirty", native: "สกปรก", pronunciation: "sòk-ga-bpròk" },
  { english: "Beautiful", native: "สวย", pronunciation: "sŭai" },
  { english: "Handsome", native: "หล่อ", pronunciation: "lòr" },
  { english: "Near", native: "ใกล้", pronunciation: "glâi" },
  { english: "Far", native: "ไกล", pronunciation: "glai" },
];

export const verbs1 = [
  { english: "Want", native: "อยาก", pronunciation: "yàak" },
  { english: "Drink", native: "ดื่ม", pronunciation: "dùuem" },
  { english: "Walk", native: "เดิน", pronunciation: "dern" },
  { english: "Run", native: "วิ่ง", pronunciation: "wîng" },
  { english: "Sit", native: "นั่ง", pronunciation: "nâng" },
  { english: "Sleep", native: "นอน", pronunciation: "nawn" },
  { english: "Stand", native: "ยืน", pronunciation: "yuuen" },
  { english: "Talk", native: "คุย", pronunciation: "kui" },
  { english: "Listen", native: "ฟัง", pronunciation: "fang" },
  { english: "Hear", native: "ได้ยิน", pronunciation: "dâi-yin" },
  { english: "Help", native: "ช่วย", pronunciation: "chûay" },
  { english: "Buy", native: "ซื้อ", pronunciation: "súue" },
  { english: "Sell", native: "ขาย", pronunciation: "khǎai" },
  { english: "Open / Turn on", native: "เปิด", pronunciation: "pèrt" },
  { english: "Close / Turn off", native: "ปิด", pronunciation: "pìt" },
  { english: "Play", native: "เล่น", pronunciation: "lên" },
  { english: "Drive", native: "ขับ", pronunciation: "khàp" },
  { english: "Ride", native: "ขี่", pronunciation: "khìi" },
  { english: "Learn", native: "เรียน", pronunciation: "rian" },
  { english: "Read", native: "อ่าน", pronunciation: "àan" },
  { english: "Write", native: "เขียน", pronunciation: "khǐan" },
  { english: "Teach", native: "สอน", pronunciation: "sǎawn" },
];

export const menuItems = [
  { code: "thaiNumbers20", label: "Numbers 0 - 20" },
  { code: "thaiNumbers100", label: "Numbers 20 - 100" },
  { code: "thaiNumbersTest", label: "Numbers (Quiz)" },
  { code: "places", label: "Places" },
  { code: "places-test", label: "Places (Quiz)" },
  { code: "pronouns", label: "Pronouns" },
  { code: "verbs1", label: "Verbs I" },
  { code: "verbs1-quiz", label: "Verbs I (Quiz)" },
  { code: "adjectives1", label: "Adjectives I" },
  { code: "adjectives1-quiz", label: "Adjectives I (Quiz)" },
  { code: "comparison", label: "Comparison" },
  { code: "random1", label: "Random I" },
  { code: "random1quiz", label: "Random I (Quiz)" },
];

export const comparisons_examples = [
  {
    text: "พิซซ่า อร่อย กว่า ข้าวผัด",
    translation: "Pizza is more delicious than fried rice",
  },
  {
    text: "แอปเปิ้ล ถูก กว่า ทุเรียน",
    translation: "Apples are cheaper than durians",
  },
  {
    text: "เชียงใหม่ หนาว กว่า กรุงเทพฯ",
    translation: "Chiang Mai is colder than Bangkok",
  },
  {
    text: "เขา สูง กว่า ฉัน",
    translation: "He is taller than me",
  },
  {
    text: "วันนี้ ร้อน กว่า เมื่อวาน",
    translation: "Today is hotter than yesterday",
  },
  {
    text: "เธอ สวย กว่า ฉัน",
    translation: "She is more beautiful than me",
  },
];

export const random1 = [
  { english: "I", native: "ผม", pronunciation: "phŏm" },
  { english: "You", native: "คุณ", pronunciation: "khun" },
  { english: "She / Her", native: "เธอ", pronunciation: "thoe" },
  { english: "We / Us", native: "เรา", pronunciation: "rao" },
  { english: "Mom", native: "แม่", pronunciation: "mâe" },
  { english: "Friend", native: "เพื่อน", pronunciation: "pêuan" },
  { english: "Home", native: "บ้าน", pronunciation: "bâan" },
  { english: "Work", native: "ทำงาน", pronunciation: "tham ngaan" },
  { english: "Game", native: "เกม", pronunciation: "geem" },
  {
    english: "Hospital",
    native: "โรงพยาบาล",
    pronunciation: "rohng-phá-yaa-baan",
  },
  { english: "Airport", native: "สนามบิน", pronunciation: "sà-năam-bin" },
  {
    english: "Toilet / Bathroom",
    native: "ห้องน้ำ",
    pronunciation: "hông-nám",
  },
  {
    english: "Cinema / Movie Theater",
    native: "โรงหนัง",
    pronunciation: "rohng nǎng",
  },
  { english: "Library", native: "ห้องสมุด", pronunciation: "hông-sà-mùt" },
  { english: "Language", native: "ภาษา", pronunciation: "paa-săa" },
  { english: "Thailand", native: "ประเทศไทย", pronunciation: "bprà-têet thai" },
  {
    english: "England",
    native: "ประเทศอังกฤษ",
    pronunciation: "bprà-têet ang-grìt",
  },
  { english: "Story", native: "เรื่อง", pronunciation: "rûeang" },
  { english: "Short (length/time)", native: "สั้น", pronunciation: "sân" },
  { english: "Short (height, person)", native: "เตี้ย", pronunciation: "dtîa" },
  { english: "Bank", native: "ธนาคาร", pronunciation: "tha-naa-khaan" },
  { english: "Restaurant", native: "ร้านอาหาร", pronunciation: "ráan aa-hăan" },
  { english: "Family", native: "ครอบครัว", pronunciation: "krôp-krua" },
  { english: "Temple", native: "วัด", pronunciation: "wát" },
  { english: "Beautiful", native: "สวย", pronunciation: "sŭay" },
  { english: "Very", native: "มาก", pronunciation: "mâak" },
  { english: "Better than", native: "กว่า", pronunciation: "gwàa" },
  { english: "Not", native: "ไม่", pronunciation: "mâi" },
  { english: "And", native: "และ", pronunciation: "láe" },
  { english: "Because", native: "เพราะว่า", pronunciation: "práw wâa" },
  { english: "Every / All", native: "ทุก", pronunciation: "túk" },
  { english: "Yesterday", native: "เมื่อวาน", pronunciation: "mûa-waan" },
  { english: "Tomorrow", native: "พรุ่งนี้", pronunciation: "prûng-níi" },
  { english: "At home", native: "ที่บ้าน", pronunciation: "thîi bâan" },
  { english: "Going to / Will", native: "จะ", pronunciation: "jà" },
  { english: "With", native: "กับ", pronunciation: "gàp" },
  { english: "Stop", native: "หยุด", pronunciation: "yùt" },
  { english: "Soon", native: "เร็ว ๆ นี้", pronunciation: "rêu-rêu níi" },
  { english: "Like (verb)", native: "ชอบ", pronunciation: "chôp" },
  { english: "Love", native: "รัก", pronunciation: "rák" },
  {
    english: "Not really / Don't really",
    native: "ไม่ค่อย",
    pronunciation: "mâi khôi",
  },
  { english: "Live (reside)", native: "อยู่", pronunciation: "yùu" },
  { english: "Listen", native: "ฟัง", pronunciation: "fang" },
  { english: "Hear", native: "ได้ยิน", pronunciation: "dâi-yin" },
  { english: "Talk", native: "คุย", pronunciation: "kui" },
  { english: "Play", native: "เล่น", pronunciation: "lên" },
];
